...
async def _exercise_once(client: httpx.AsyncClient, *, content: str, execution_mode: str) -> tuple[bool, float, str | None]:
    started = perf_counter()
    try:
        create_response = await client.post("/api/router/v2/sessions", json={})
        if create_response.status_code != 201:
            return False, (perf_counter() - started) * 1000, f"create_session:{create_response.status_code}"
        ...
        if message_response.status_code != 200:
            return False, (perf_counter() - started) * 1000, f"message:{message_response.status_code}"
        ...
        if not isinstance(current_graph, dict):
            return False, (perf_counter() - started) * 1000, "missing_current_graph"
        if current_graph.get("status") != "ready_for_dispatch":
            return False, (perf_counter() - started) * 1000, f"graph_status:{current_graph.get('status')}"
    ...
    *,
    ...
    async def worker() -> None:
        ...
        while await should_continue():
            ok, latency_ms, error_key = await _exercise_once(
                client,
                content=content,
