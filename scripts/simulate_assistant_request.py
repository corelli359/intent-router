#!/usr/bin/env python3
"""Simulate assistant-service requests against the deployed assistant API.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Simulate assistant-service requests.")
    parser.add_argument(
        "--base-url",
        default=DEFAULT_BASE_URL,
        help=f"Assistant base URL. Default: {DEFAULT_BASE_URL}.",
    )
    parser.add_argument("--session-id", default=f"assistant_sim_{int(time.time())}", help="Session id.")
    ...
    mode_group.add_argument("--stream", dest="stream", action="store_true", help="Use /api/assistant/run/stream.")
    mode_group.add_argument("--no-stream", dest="stream", action="store_false", help="Use /api/assistant/run.")
...
def main() -> int:
    ...
    if args.stream:
        url = f"{args.base_url.rstrip('/')}/api/assistant/run/stream"
        print(f"POST {url}")
        start = time.time()
    ...
    url = f"{args.base_url.rstrip('/')}/api/assistant/run"
    print(f"POST {url}")
