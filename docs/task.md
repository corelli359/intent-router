...
### [x] T33 · V2 隐式条件依赖修复
设计：如果某条条件边依赖的字段并不由当前上游节点产出，不能把责任推给 prompt“碰运气”，而要在 runtime 里做语义修复。像“我想给小明转账1000元，如果卡里余额还剩超过2000，我就换100美元”这种输入，本质上隐含了一步“转账后再查余额”。
实现：新增 `router_core/v2_graph_semantics.py`，给 V2 增加条件字段别名解析、内置 intent 上下文字段注册，以及不可执行条件边的 repair 逻辑；当条件边引用的字段并非当前 source node 可产出时，runtime 会自动插入必要的隐式 producer 节点（例如补一个 `query_account_balance`），而不是直接把后继节点误判为条件不满足。`prompt_templates.py` 同步补了 hidden-node 指令，相关回归测试已加到 `test_router_api_v2.py`。
