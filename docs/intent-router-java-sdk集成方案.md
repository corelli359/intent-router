...
│  │  │                            │  │  │ uvicorn               │  │
│  │  │  RouterEventListener       │  │  │                      │  │
│  │  │  └── onEvent(event)        │◀─┼──│ POST /api/router/v2 │  │
│  │  │                            │  │  │      /sdk/process   │  │
│  │  │                            │  │  │      ↓               │  │
│  │  └────────────────────────────┘  │  │ orchestrator 全链路   │  │
│  │                                  │  │      ↓               │  │
...
Java 业务代码调用 SDK
  ...
  → Python /api/router/v2/sdk/process 端点接收
...
新增一个 `/api/router/v2/sdk/process` 端点，**接收外部传入的完整 session 状态**，处理完后**返回更新后的 session**。
...
> [!IMPORTANT]
> `session_state` 在 Python 侧的 HTTP 契约是 **JSON object**，不是 JSON 字符串。
> Java SDK 对外可以继续暴露 `String sessionState`，但在真正发 HTTP 请求前必须先反序列化成对象节点再放入 body；
> `guided_selection`、`proactive_recommendation`、`recommendation_context` 也一样，避免出现双重 JSON 编码。

**端点实现要点**：

```python
@sdk_router.post("/api/router/v2/sdk/process")
async def process_message(request: ProcessRequest, ...):
    # ① 反序列化外部 session；空对象时初始化
    ...
    if request.cust_id and session.cust_id != request.cust_id:
        raise HTTPException(status_code=400, detail="cust_id does not match session_state.cust_id")
    
    # ② 首期 demo 用 cust_id 级锁，避免共享内存污染
    async with sdk_request_lock(session.cust_id):
        orchestrator.session_store._sessions[session.session_id] = session
        orchestrator.session_store.long_term_memory.replace_facts(
        ...
        async def event_generator():
            ...
            try:
                ...
                orchestrator.session_store.long_term_memory.clear_facts(session.cust_id)
```
...
上面这套方案能复用现有 orchestrator，但有 4 个必须写进实现的约束：

1. **空 session 不能直接 `model_validate({})`**  
   当前 `GraphSessionState` 的 `session_id` / `cust_id` 是必填字段，空对象会直接触发 `ValidationError`。首次请求必须显式创建新 session。

2. **`cust_id` 必须和 `session_state.cust_id` 一致**  
   当前 `message_flow` / `action_flow` 在入口第一步都会调用 `session_store.get_or_create(session_id, cust_id)`。如果二者不一致，已有 session 会被替换成一个新的空 session，导致上下文丢失。

3. **不能只“写入” long-term memory，不做清理**  
   当前 `LongTermMemoryStore` 是运行时共享对象。如果 SDK 端点只做 `replace_facts()` 而不在请求结束后清理，Python 进程就不再是无状态；后续请求可能读到上一次注入的记忆。

4. **最终 `session_state` 不能在 async generator 的 `finally` 里 `yield`**  
   一旦客户端断连或生成器提前关闭，在 `finally` 再 `yield` 存在协议风险。更稳妥的做法是在 `task.done() and queue.empty()` 后，先显式发送最后一帧，再退出循环。

### 2.4 LongTermMemoryStore 需要的小改造
...
def clear_facts(self, cust_id: str) -> None:
    """Remove one customer's injected facts after an SDK request finishes."""
    self._customers.pop(cust_id, None)
```

> [!NOTE]
> 更彻底的方案是“每个 SDK 请求创建 request-scoped `GraphSessionStore` / `GraphRouterOrchestrator`”，从根上消除共享状态。
> 但对首期 demo 来说，先用 `cust_id` 级锁 + `replace_facts()/clear_facts()` 能把改动控制在 API 层。
...
| `core/support/memory_store.py` | 增加 `replace_facts()` / `clear_facts()` | 30 分钟 |
...
public class ProcessMessageRequest {
    ...
    private String recommendationContext; // JSON
...
public class ProcessActionRequest {
    ...
    private String custId;
    private List<String> longTermMemory;
    private String source;             // "router" / "graph"
...
> [!IMPORTANT]
> Java SDK 对外模型可以保留 `String sessionState` / `String guidedSelection` 这种简洁接口；
> 但 SDK 内部构造 HTTP body 时必须把它们转成 JSON object，而不是把字符串原样塞进请求体。
> 否则 Python 侧 `session_state: dict[str, Any]` 会直接校验失败。
...
public class IntentRouterClientImpl implements IntentRouterClient {
    ...
    public RouterResult processMessage(ProcessMessageRequest request) {
        ...
        webClient.post()
            .uri("/api/router/v2/sdk/process")
            .accept(MediaType.TEXT_EVENT_STREAM)
            .bodyValue(buildPayload(request))
            .retrieve()
            .bodyToFlux(new ParameterizedTypeReference<ServerSentEvent<String>>() {})
            .doOnNext(sse -> {
                RouterEvent event = parseSseEvent(sse);
...
后续如果引入“子智能体”或“总控智能体”，推荐按下面这条边界落：

- **Java 只调用 Router SDK**  
  Java 负责 session 持久化、长时记忆装载、前端事件转发，不直接调业务 Agent。

- **Router 调用需要进入 Router 状态机的意图级 Agent**  
  只要这个能力需要被纳入 `session_state`、graph 节点、统一取消、统一确认、统一 SSE 事件，就应该注册成 Router 的叶子 intent，由 Router 通过 `agent_client` 调用。

- **叶子业务 Agent 可以在内部再调自己的子能力**  
  如果这些子能力只是该 intent 的实现细节，例如内部 workflow、RAG、工具调用、同域子 Agent 编排，并不需要独立的 session / graph 节点 / 前端确认卡片，那么由业务 Agent 自己调用更合理。

- **不要让 Router 和总控 Agent 同时做全局分发**  
  “跨意图识别、规划、节点编排”这一层只能有一个总控。对当前 demo 原型，建议继续由 Router 做总控；如果未来必须引入 `OneAgent` 这类总控智能体，那么它要么替代 Router 的规划层，要么退化成某一个叶子业务 Agent，不能双总控并存。

一句话判断标准：

- **需要进入 Router 的状态机，就让 Router 调。**
- **只是某个业务 Agent 的内部实现，就让 Agent 自己调。**

### 3.7 调用时序文档

完整时序关系单独整理在：

- [intent-router-java-sdk调用时序设计.md](/root/intent-router/docs/intent-router-java-sdk调用时序设计.md)
...
> [!IMPORTANT]
> 这里的 `"{}"` 只是 Java SDK 的对外输入形式。
> SDK 在真正发 HTTP 前，应把它转成空 JSON object，而不是字符串 `"{}"`。

---

## 五、SSE 事件协议

Python → Java 的 SSE 流中，事件类型与当前基本一致，**额外加一个只给 SDK 内部消费的结束帧**：
...
> 所以即使保留 `session.idle`，仍然需要单独增加 `session_state` 结束帧给 Java 落库。

### 5.1 Java 侧 SSE 解析补充

Java 侧不要把 `text/event-stream` 当作普通 chunked 字符串流来拆。更稳妥的做法是使用原生 SSE 解码器：
...
- SSE 帧边界不等于 TCP/HTTP chunk 边界，`bodyToFlux(String.class)` 不能保证一次回调就是一个完整 event。
- `session_state` 结束帧可能比较大，手写按行拼接比直接交给 SSE decoder 更容易出错。
...
> [!CAUTION]
> 上面这个结论只在“请求内没有共享可变状态”时完全成立。
> 如果首期 demo 采用“把 session / memory 临时注入到共享 `session_store`”的方式，那么**同一个 `cust_id` 的并发请求仍然需要串行化**，否则会出现：
> - A 请求覆盖 B 请求注入的 long-term memory
> - 同一个 session 在两个请求里同时被改写
> - 事件流和最终 session_state 对不上

### 6.3 Session 无状态 = 无亲和性要求

因为 session 每次由 Java 传入，Python Worker 之间**原则上不共享业务状态**。
uvicorn 的 `--workers 3` 会自动做 round-robin 负载均衡，无需 session 亲和性。

但首期 demo 如果继续复用共享 `session_store` 注入法，仍然要接受一个现实约束：

- **无 session 亲和性**：可以，不要求同一个 session 固定命中同一个 worker
- **同客并发串行化**：需要，在单个 worker 内避免共享状态互相污染
- **Java 侧自己做最终状态持久化**：需要，Python 不保存 session
...
| ④ `memory_store.py` 增加 `replace_facts()` / `clear_facts()` | `core/support/memory_store.py` | 🟡 |
| ⑤ `app.py` 挂载 sdk_router | `api/app.py` | 🟢 |
| ⑥ 同一个 `cust_id` 并发冲突测试 | `tests/api/test_sdk_routes.py` | 🔴 |
| ⑦ 手动联调 curl / WebClient SSE 测试 | - | 🟡 |
...
SDK 嵌入模式（新增）：
  ...
  Java 调 /api/router/v2/sdk/process（新端点）
