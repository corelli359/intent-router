...
def test_multi_intent_serial_flow_waits_then_resumes() -> None:
    ...
    create_response = client.post("/api/router/sessions")
...
def test_transfer_task_supports_multiple_rounds() -> None:
    client = TestClient(create_router_app())
    session_id = client.post("/api/router/sessions").json()["session_id"]
...
def test_expired_session_promotes_short_term_memory_to_customer_memory() -> None:
    ...
    create_response = client.post("/api/router/sessions", json={"cust_id": "cust_001"})
    session_id = create_response.json()["session_id"]
