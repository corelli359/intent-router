...
DEFAULT_RECOGNIZER_SYSTEM_PROMPT = (
    ...
    "对转账类意图尤其要注意：收款人姓名、昵称、关系称呼、金额、卡号都只是槽位，不是识别 intent 的前提条件。"
    "只要当前消息明确表达了转账/汇款/打款/付款动作，即使缺少金额或收款人，也应优先识别为转账意图，而不是返回空列表。"
    "像“我要转账”“帮我转账”“给小明转账”“转200给我妈”都应该识别为一个转账 intent。"
    "已注册 intent 的 examples、routing_examples，以及 field_catalog / slot_schema 里的 examples，只用于帮助你理解语义边界。"
    "你绝不能把这些示例里的姓名、金额、卡号直接复制到当前识别结果里。"
    "如果当前消息没有明确出现收款对象，不要因为示例、常识或联想擅自补出人名。"
    "像“我要转账”“帮我转账”只能说明是转账意图，不能推出收款人是谁。"
...
DEFAULT_SLOT_EXTRACTOR_SYSTEM_PROMPT = (
    ...
    "人名识别必须严格以当前消息或当前节点原始片段中的显式证据为准。"
    "当文本出现“给X转账”“转给X”“汇给X”“打给X”“给X转200”“收款人X”这类结构时，只要 X 明显是正式姓名、昵称、关系称呼或用户对人的明确称谓，就应抽取到收款人相关槽位。"
    "当文本只说“我要转账”“帮我转账”“转一笔钱”，没有出现明确收款对象时，必须让 payee_name 保持缺失，不能从 examples、历史示例、模型常识或联想里补出“小明”“王芳”“我妈”等人名。"
    "对于中文两到四字姓名、复姓姓名、昵称、关系称呼、带轻微修饰的人名表达，例如“小明”“王芳”“欧阳娜娜”“我妈”“老张”“王芳姐”，如果当前句子明确把它当作转账对象，就应保留用户原文并抽取。"
    "已注册的 examples、counter_examples、prompt_hint 只是 schema 约束，不得把其中的样例值照抄到当前输出。"
...
DEFAULT_UNIFIED_GRAPH_BUILDER_SYSTEM_PROMPT = (
    ...
    "对转账类消息尤其要注意：只要当前消息明确表达了转账动作，即使金额或收款人缺失，也应保留一个转账 primary intent 和一个转账节点，等待后续补槽，不得因为缺少人名或金额而 no-match。"
    "如果当前消息没有明确出现收款对象，例如“我要转账”“帮我转账”，允许生成转账节点，但 payee_name 必须为空。"
    "如果当前消息明确出现转账对象，例如“给小明转账”“转200给我妈”“给王芳汇款”，应保留用户原文对象称呼并写入对应的 payee_name。"
    "node.slot_memory 中的人名只能来自当前消息显式证据，不能复制 intent examples、field examples、slot examples，也不能凭常识补全。"
